...
3b. **Sign in with Apple has TWO separate configurations — native (app) and web (storefront) — and they are not interchangeable.** The iOS app uses the *native* flow (`sign_in_with_apple` + `signInWithIdToken`), keyed on the App ID `com.anuragrishi.knowthyself`. The public storefront sign-in (`admin/src/app/(shop)/store/signin/page.tsx`) uses the *web* flow (`supabase.auth.signInWithOAuth({ provider: 'apple' })`, returning via `/auth/callback`), added Sep 2026 alongside the existing Google button so an Apple app-user can also sign in on the web to buy. The web flow needs config the native one does not, and **without it the button errors with "Unsupported provider: provider is not enabled"**:

   - **Apple Developer → a Services ID** (a *separate* identifier from the App ID, e.g. `com.anuragrishi.knowthyself.web`) with **Sign in with Apple** enabled, its **Domain** = `pay.anuragrishi.com` (+ the Supabase project domain) and its **Return URL** = `https://<project-ref>.supabase.co/auth/v1/callback`.
   - **A Sign in with Apple key (.p8)** → note **Key ID** + **Team ID**.
   - **Supabase → Authentication → Providers → Apple**: enable, and enter the **Services ID** (as client id), **Team ID**, **Key ID** and the **.p8** — Supabase generates the client secret from these.
   - **Group the Services ID under the primary App ID** (`com.anuragrishi.knowthyself`) so the Apple user identifier is the SAME across app and web; otherwise a buyer who used Apple in the app comes back as a stranger on the web and their purchase lands on a second account. This is the Apple analogue of the "keep Firebase + Google on one project" rule above.
   - Apple's own quirks apply either way: the name/email are returned only on the FIRST authorization, and a user may choose Hide My Email (a `@privaterelay.appleid.com` address) — the stable `sub` still resolves them to one account as long as the grouping above is done.
   - The **Apple client secret (JWT) expires at most every 6 months.** Supabase regenerates it from the stored .p8, so this is not a manual rotation — but if the .p8 is ever revoked in the Apple console, web Apple sign-in stops until a new key is added in both places.
...
**A few more device-test fixes rounded out the run:**

* **iOS offline playback `-1004`, take two.** A submitted 2.2.2 build (build 36)
  still showed `-1004` on offline files. Since the version string only became
  2.2.2 *after* the `NSAllowsLocalNetworking` fix, that build already had it — so
  the flag alone was not enough for AVPlayer to reach the raw `127.0.0.1` proxy.
  Added an explicit `NSExceptionDomains` cleartext exception for `127.0.0.1` and
...
## 17. Crash reporting — Firebase Crashlytics added — 21 September 2026

**The problem it solves.** Reports came in that "some devices can't launch the
app," but there was **no way to see why**: the app had **no crash reporter at
all** (Section 7's scaling plan listed Sentry, never started). Store-device
crashes were invisible. Play Console's **Android vitals → Crashes and ANRs**
does collect native/Java crashes for store installs, but its **Overview
crash-rate needs a minimum volume** before it shows anything, and vitals only
covers users who opted into diagnostics — so a smaller install base reads as
"Not enough data." That is a Google threshold, not a misconfiguration.

**What was added (2.2.2+48).** Firebase Crashlytics, riding on the Firebase/FCM
setup that already exists (project `rishi-503917`) — no new project, no new
service account:
- `pubspec.yaml`: `firebase_crashlytics ^4.1.3` (co-resolves with
  `firebase_core ^3.6.0` / `firebase_messaging ^15.1.3`).
- `android/settings.gradle` + `android/app/build.gradle`: declare and apply the
  `com.google.firebase.crashlytics` gradle plugin **inside the existing
  `if (file("google-services.json").exists())` guard**, so a checkout without
  that file still builds (with no reporting) rather than failing — the same
  pattern google-services already uses. The gradle plugin is what captures
  **Java/Kotlin and native launch crashes automatically** and uploads the
  mapping so they symbolicate.
- `main.dart`: initialise Firebase up front, then route `FlutterError.onError`
  → `recordFlutterFatalError` and `platformDispatcher.onError` →
  `recordError(fatal: true)`. **All guarded**: a device with no Play Services /
  no google-services.json still reaches the login screen; a failure there only
  costs reporting. `Firebase.initializeApp` is idempotent, so `PushService.init`
  reusing it is fine.

**Why Crashlytics and not just vitals.** Crashlytics has **no minimum-data
threshold** — it reports every crash from every install, grouped by device
model and Android version, with a stack trace. For a "some devices" launch
crash on a modest install base, that is the only reliable source; Play vitals
stays blank until volume builds.

**Where reports appear.** Firebase Console → Crashlytics (richest: Flutter +
native), and Play Console → Android vitals → Crashes and ANRs. Both need the
build **actually shipped and running on real devices** first.

**Coverage caveat.** The gradle plugin symbolicates native/Java crashes. Dart
stack traces from a `--obfuscate --split-debug-info` release are NOT
automatically deobfuscated — that would need uploading the Dart symbol files.
Not wired up yet; the launch crashes we are chasing are native/Java, which are
covered. Revisit if Dart-level obfuscated traces ever need reading.

**Superseded.** This closes the "error tracking — Sentry, not started" item
from Section 7 / README §7. Crashlytics is the reporter of record now.

**Shipped alongside.** +48 is also the build that finally releases +44..+47 to
users — none had reached the stores: the offline-playback-jumps-to-Home fix
(+46), the offline decrypt-crash guard + Auto Backup exclusion + honest
device-lock play errors (+47), the no-slide bottom-nav tabs (+44) and the
Android-only live sessions (+45).

**Version name bumped to 2.2.3.** 2.2.2 is already published on both stores, so
this build carries a higher version STRING (`2.2.3+48`), not just a higher build
number — a store won't accept a new upload under an already-published version
name. No behaviour changed with the rename. Both places that hold the string
were moved together: `pubspec.yaml` and `AppConfig.appVersion` (support tickets
report the latter — keeping them in step is a standing rule). The build number
after the `+` is still governed by Codemagic's `$BUILD_NUMBER` on CI; `+48` here
governs local builds.

**Also, during setup:** the Crashlytics gradle plugin was removed. Its
`firebase-crashlytics-buildtools` jar is a large download that repeatedly failed
with TLS corruption (`bad_record_mac / Tag mismatch`) on a local Windows build —
almost certainly antivirus HTTPS scanning mangling the transfer. The gradle
plugin only adds build-time R8-mapping and NDK symbol upload; the
`firebase_crashlytics` package still reports Java/Kotlin and Dart crashes at
runtime without it. Removed from `settings.gradle`/`app/build.gradle` to unblock
the build; re-add on a clean network / CI if native-symbol upload is wanted.

### The first bug Crashlytics caught — the SQLITE_BUSY launch crash (+49)

Within a day of shipping 2.2.3, Crashlytics returned the exact "some devices
can't launch" crash that had been undiagnosable before, on `2.2.3 (48)`:

```
Fatal Exception: DatabaseException(database is locked (code 5 SQLITE_BUSY))
  sql 'BEGIN EXCLUSIVE'
  at CacheObjectProvider.open(cache_object_provider.dart:30)
  at new CacheStore.<fn>(cache_store.dart:34)
```

**Cause.** `flutter_cache_manager` (transitive, 3.4.1) is pulled by
`audio_service`, which loads every `MediaItem`'s remote `artUri` — the cover art
for the media/lock-screen notification — through it. `audio_player_handler`
sets `artUri` on every track (`_toMediaItem`), and `loadPlaylist` adds a whole
queue of them at once. audio_service then tries to resolve artwork for several
tracks simultaneously, so several first-time opens of the ONE SQLite image-cache
DB race, and one loses with `SQLITE_BUSY` on `BEGIN EXCLUSIVE`. It is a timing
race, which is why it hit only some devices and not every launch — exactly the
shape of the "some devices can't launch" reports.

**What the user saw.** On the OLD build (pre-2.2.3, no error handler) the
exception was uncaught, so the app **hard-crashed** — "opens then immediately
closes", "keeps stopping", or never past the splash — intermittently, on a
subset of phones, whenever audio with cover art loaded at startup. On `2.2.3
(48)` the +48 handler caught it (the report is a
`io.flutter.plugins.firebase.crashlytics.FlutterError`, i.e. it came through the
Dart handler, not a native crash), so the app **survived** — notification
artwork could be missing and the event was logged, but no crash. This is the
clearest evidence that adding the global error handler in +48 already blunted
the launch crashes before the root cause was even known.

**Fix (+49).** Declare `flutter_cache_manager` directly and open its cache store
ONCE, awaited, in `main()` before `AudioService.init`. The store is a singleton,
so once initialised every later access reuses it instead of racing a first open.
Guarded so a warmup failure can't itself block launch. +48's handler stays as
the safety net. No version-name change (still 2.2.3); build number to +49.

**Takeaway.** This is exactly why Crashlytics was added: a device-specific,
timing-dependent launch crash is nearly impossible to reproduce or reason about
from the code alone, and Play vitals stayed below its data threshold. One real
stack trace named the library, the file and the line, and the fix followed in
minutes.

---

*Last updated: 22 September 2026 — 2.2.3+49: Crashlytics caught its first real bug the day 2.2.3 shipped — the "some devices can't launch" crash was a flutter_cache_manager SQLITE_BUSY on 'BEGIN EXCLUSIVE', from audio_service loading a queue's worth of remote cover art through one SQLite image-cache DB at once (a timing race, hence only some devices); +49 warms that cache store open once, serially, in main() before AudioService.init so later accesses can't race a first open (Section 17). The +48 global error handler had already been catching it, so 2.2.3 users were surviving rather than hard-crashing. Also in setup: the Crashlytics gradle plugin was removed after its buildtools jar kept failing to download with TLS corruption (bad_record_mac) on a local Windows build — runtime crash reporting still works without it. Before that, 21 September 2026 — 2.2.3+48 (version name bumped from 2.2.2, already published on both stores, so a new upload needs a higher version string; pubspec + AppConfig.appVersion moved together): Firebase Crashlytics added (Section 17) so store-device crashes — including the "some devices can't launch" reports — are finally visible, since the app had no crash reporter at all and Play vitals stays below its data threshold on a small install base; guarded so a device with no Play Services still starts, and applied inside the google-services.json guard so other checkouts still build. This build also, at last, ships +44..+47 to users (the offline-playback-to-Home fix, the offline decrypt-crash guard + backup exclusion + honest device-lock play errors, the no-slide tabs, and Android-only live sessions) — none had been released. This session also, in the admin: name/email search in Users, explicit "Make premium"/"Make free" actions, edit-audio/video-details-after-upload with cover upload, and Sign in with Apple on the storefront (config recorded in Section 10, item 3b). Before that, 12 September 2026 — 2.2.2 (builds through +43): the External Link Account link, finally landed after three App Store rejections (Section 16) — build 38 rejected because a self-drawn dialog + url_launcher does not satisfy the entitlement (it needs StoreKit's `ExternalLinkAccount.open()`); a native MethodChannel added for it, plus a manual-signing detour (openssl, no Mac) because the managed capability cannot be added to an API-created profile; the button then "did nothing" because the channel was registered in AppDelegate while the app uses the UIScene lifecycle, so `window.rootViewController` was nil there — moved to `SceneDelegate.scene(willConnectTo:)`; build 48 rejected again because the in-app affordance must be a standard HTML-style link showing the URL/domain matching Info.plist, not a neutral "Manage your account" button — now a Netflix-style bordered box showing `https://pay.anuragrishi.com/store`; and the link deliberately kept in Settings, never on the lock screen, to preserve the reader-app framing. Also an opaque fill for the previously see-through locked bottom sheet. Before that, 8 September 2026 — 2.2.2 (builds through +31): the offline-downloads work — the vanishing-downloads investigation (the revoke purge was user-scoped, and the real cause was the access-lapsed purge deleting FREE downloads as well as premium — now `purgePremiumDownloads` keeps free content, plus migration 20260908000001 restores a re-registering device's downloads), download retry/resume on a dropped connection with friendly errors instead of the raw signed URL, the `openNowPlaying` single-open guard against stacked player screens, the second iOS `-1004` fix (explicit 127.0.0.1/localhost ATS exception), the cover aspect-ratio decision (audio 1:1, courses 16:9, with admin size hints), per-file metadata in bulk audio upload, and the login one-device note moved above the fold. The version briefly read 2.3.1 before reverting to 2.2.2 to match the developer-rejected App Store Connect entry. Before that, 31 August 2026 — 2.2.1: the account-wide Razorpay webhook fix (a live-disabled webhook), the iOS `-11828`/`-1004` playback fixes and the audio double-open, the admin Help & Support console and bulk audio upload, and — the headline — Apple GRANTING the External Link Account Entitlement (reversing the 27 August denial in Section 14), the iOS account link built for it, the Google-on-web sign-in that unblocked Google users from buying, and Resend custom SMTP for email. See Section 15. Before that, 27 August 2026 — 2.2.0: Help & Support, the course resume card, offline-player artwork and skip controls, the download-purge fix (hasLapsed vs hasAccess), and the dependency plan in Section 13. Before that, 25 August 2026, the day the App Store approved 2.1.1 as a reader app. That session also produced the purple-glass restyle, the image-decode and upload-resize work, and the full codebase audit in Section 12 — which found a critical entitlement hole that had been open since June. Before that: the App Store 3.1.1 rejection on 12 August — the iOS reader-app build and the public storefront it forced. Before that: live sessions, push notifications and the fan-out scaling work (2 August); Phases 3b, 4 and 5 landed in one extended session earlier still. See the bug-fix chronology at the end of Section 7 for what broke along the way and why.* — 2.2.2 (builds through +43): the External Link Account link, finally landed after three App Store rejections (Section 16) — build 38 rejected because a self-drawn dialog + url_launcher does not satisfy the entitlement (it needs StoreKit's `ExternalLinkAccount.open()`); a native MethodChannel added for it, plus a manual-signing detour (openssl, no Mac) because the managed capability cannot be added to an API-created profile; the button then "did nothing" because the channel was registered in AppDelegate while the app uses the UIScene lifecycle, so `window.rootViewController` was nil there — moved to `SceneDelegate.scene(willConnectTo:)`; build 48 rejected again because the in-app affordance must be a standard HTML-style link showing the URL/domain matching Info.plist, not a neutral "Manage your account" button — now a Netflix-style bordered box showing `https://pay.anuragrishi.com/store`; and the link deliberately kept in Settings, never on the lock screen, to preserve the reader-app framing. Also an opaque fill for the previously see-through locked bottom sheet. Before that, 8 September 2026 — 2.2.2 (builds through +31): the offline-downloads work — the vanishing-downloads investigation (the revoke purge was user-scoped, and the real cause was the access-lapsed purge deleting FREE downloads as well as premium — now `purgePremiumDownloads` keeps free content, plus migration 20260908000001 restores a re-registering device's downloads), download retry/resume on a dropped connection with friendly errors instead of the raw signed URL, the `openNowPlaying` single-open guard against stacked player screens, the second iOS `-1004` fix (explicit 127.0.0.1/localhost ATS exception), the cover aspect-ratio decision (audio 1:1, courses 16:9, with admin size hints), per-file metadata in bulk audio upload, and the login one-device note moved above the fold. The version briefly read 2.3.1 before reverting to 2.2.2 to match the developer-rejected App Store Connect entry. Before that, 31 August 2026 — 2.2.1: the account-wide Razorpay webhook fix (a live-disabled webhook), the iOS `-11828`/`-1004` playback fixes and the audio double-open, the admin Help & Support console and bulk audio upload, and — the headline — Apple GRANTING the External Link Account Entitlement (reversing the 27 August denial in Section 14), the iOS account link built for it, the Google-on-web sign-in that unblocked Google users from buying, and Resend custom SMTP for email. See Section 15. Before that, 27 August 2026 — 2.2.0: Help & Support, the course resume card, offline-player artwork and skip controls, the download-purge fix (hasLapsed vs hasAccess), and the dependency plan in Section 13. Before that, 25 August 2026, the day the App Store approved 2.1.1 as a reader app. That session also produced the purple-glass restyle, the image-decode and upload-resize work, and the full codebase audit in Section 12 — which found a critical entitlement hole that had been open since June. Before that: the App Store 3.1.1 rejection on 12 August — the iOS reader-app build and the public storefront it forced. Before that: live sessions, push notifications and the fan-out scaling work (2 August); Phases 3b, 4 and 5 landed in one extended session earlier still. See the bug-fix chronology at the end of Section 7 for what broke along the way and why.*
