...
| `STOREFRONT_URL` | `send-content-notifications` (via `_shared/n8n.ts`) | Origin of the public storefront, e.g. `https://rishi-app2.vercel.app`. Appended with `/store` and sent as `renew_url` on every expiry message. **This is the entire renewal path for an iOS member**: membership is a manually renewed grant, and the iOS build carries no purchase UI under Guideline 3.1.3(a), so there is no in-app way to renew and the app may not link to one. Absent, the message still sends and simply has nowhere to renew — logged, not silent |
