...
class LmsRemoteDataSource {
  ...
  Future<List<Map<String, dynamic>>> getCourses() async {
    final rows = await _client
        .from('courses')
        .select(
          'id, title, description, cover_image_url, is_premium, sort_order, '
  ...
  Future<Map<String, dynamic>?> getCourse(String courseId) async {
    return await _client
        .from('courses')
        .select('id, title, description, cover_image_url, is_premium')
        .eq('id', courseId)
        .maybeSingle();
  }
  ...
  Future<List<Map<String, dynamic>>> getModules(String courseId) async {
    final rows = await _client
        ...
        .select(
          'id, title, position, '
          'lessons(id, title, description, lesson_type, body_markdown, position, '
